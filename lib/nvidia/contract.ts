...
async function fetchWithRetry(url: string, options: RequestInit, retries = 2, timeoutMs = 60000): Promise<Response> {
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      console.warn(`[contract-ai] tentativo ${attempt + 1} fallito con status ${response.status}, riprovo...`)
    } catch (error) {
      clearTimeout(timeout)
      if (attempt === retries) throw error
      console.warn(`[contract-ai] tentativo ${attempt + 1} fallito, riprovo...`, error)
