...
import { Zap, FolderOpen, History, CreditCard, ArrowRight, Sparkles, Loader2 } from 'lucide-react'
...
export default function DashboardOverviewPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-white/10 p-6 shadow-2xl shadow-cyan-500/10 backdrop-blur-xl sm:p-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 p-5">
            ...
            <p className="text-3xl font-semibold text-white">{profile ? profile.credits : '...'}</p>
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-xl shadow-black/20">
        ...
        {events === null && (
          <div className="flex justify-center py-8">
            <Loader2 className="h-5 w-5 animate-spin-fast text-slate-500" />
          </div>
        )}
        ...
        {events && events.length > 0 && (
          <div className="space-y-2">
            {events.map((event) => (
              <div key={event.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm">
                <div>
...
function QuickLink({ href, icon: Icon, title, description }: { href: string; icon: typeof FolderOpen; title: string; description: string }) {
  return (
    <Link
      href={href}
      className="group flex items-center justify-between rounded-2xl border border-white/10 bg-slate-900/80 p-5 shadow-xl shadow-black/20 transition-colors duration-150 ease-out hover:border-cyan-400/30 hover:bg-slate-900"
    >
      ...
      <ArrowRight className="h-4 w-4 text-slate-500 transition-colors duration-150 ease-out group-hover:translate-x-1 group-hover:text-cyan-300" />
    </Link>
  )
}
