    ...
    <div className="flex min-h-screen flex-col bg-[#050506] text-white">
      ...
      <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-4 flex items-center justify-between">
          <p className="text-sm text-neutral-400">{t('chatAiPage.freeConversation')}</p>
          {messages.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleReset}
                className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 py-1.5 text-xs text-neutral-300 transition-colors duration-150 ease-out hover:bg-white/[0.06] hover:text-white"
              >
        ...
        <div className="flex min-h-[28rem] flex-1 flex-col rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4 shadow-xl shadow-violet-500/10 backdrop-blur-xl sm:p-6">
          <div className="flex-1 space-y-5 overflow-y-auto pr-1">
            {messages.length === 0 && (
              <div className="flex h-full flex-col items-center justify-center gap-3 py-16 text-center text-neutral-600">
            ...
            {loading && (
              ...
              >
                ...
                <div className="flex items-center gap-2 rounded-2xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-neutral-400">
                  <TypingIndicator />
                </div>
              </motion.div>
            )}
          ...
          <form onSubmit={handleSend} className="mt-4 flex gap-2 border-t border-white/[0.08] pt-4">
            <input
              type="text"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder={t('chatAiPage.inputPlaceholder')}
              className="flex-1 rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-sm text-white placeholder-neutral-500 focus:border-violet-400/50 focus:outline-none"
            />
            <button
              type="submit"
    ...
    >
      ...
      >
        ...
        {!isUser && (
          <button
            onClick={handleCopy}
            className="absolute -right-9 top-2 rounded-lg bg-white/[0.03] p-1.5 text-neutral-400 opacity-0 transition-colors duration-150 ease-out hover:bg-white/[0.06] hover:text-white active:scale-90 group-hover:opacity-100"
